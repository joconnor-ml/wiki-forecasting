...
def get_features(df):
    ...
    #features["ewm1"] = df.ewm(halflife=30, axis=1).mean().iloc[:, -1]
    #features["ewm2"] = df.ewm(halflife=5, axis=1).mean().iloc[:, -1]
    ...
    #features["med_year"] = df.iloc[:, ::-365].median(axis=1)
...
def prepare_data(prediction_gap):
    ...
    X = []
    y = []
    for i, date in enumerate(df.columns[-365:]):
        y.append(df[date])
        a = df.loc[:, :date].iloc[i::365, :-prediction_gap]
    ...
    return X.loc[train], y.loc[train], X.loc[~train], y.loc[~train], df
